...
exports.createBridge = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      'INSERT INTO bridges (name, location, lng, lat, image_url) VALUES (?, ?, ?, ?, ?)',
      [name, location, lng, lat, image_url]
    )
...
exports.updateBridge = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      'UPDATE bridges SET name = ?, location = ?, lng = ?, lat = ?, image_url = ? WHERE id = ?',
      [name, location, lng, lat, image_url, id]
    )
...
exports.getSectionById = async (req, res) => {
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({ success: false, msg: '断面不存在' })
    }
...
exports.createSection = async (req, res) => {
  try {
    ...
    if (!bridge_id || !name) {
      return res.status(400).json({ success: false, msg: '桥梁ID和断面名称不能为空' })
    }
    
    const [result] = await db.query(
      'INSERT INTO sections (bridge_id, name, description, lng, lat, pos_x, pos_y, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [bridge_id, name, description, lng, lat, pos_x, pos_y, image_url]
    )
...
exports.updateSection = async (req, res) => {
  try {
    ...
    if (!name) {
      return res.status(400).json({ success: false, msg: '断面名称不能为空' })
    }
    
    const [result] = await db.query(
      'UPDATE sections SET bridge_id = ?, name = ?, description = ?, lng = ?, lat = ?, pos_x = ?, pos_y = ?, image_url = ? WHERE id = ?',
      [bridge_id, name, description, lng, lat, pos_x, pos_y, image_url, id]
    )
...
exports.getSensorsBySection = async (req, res) => {
  try {
    ...
    if (!sectionId) {
      return res.status(400).json({ success: false, msg: '断面ID不能为空' })
    }
...
exports.createSensor = async (req, res) => {
  try {
    ...
    if (!section_id || !sensor_code || !sensor_name || !sensor_type) {
      return res.status(400).json({ success: false, msg: '断面ID、传感器编码、名称和类型不能为空' })
    }
    ...
    const [result] = await db.query(
      'INSERT INTO sensors (section_id, sensor_code, sensor_name, sensor_type, limit_max, limit_min, unit, pos_x, pos_y) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [section_id, sensor_code, sensor_name, sensor_type, limit_max, limit_min, unit, pos_x, pos_y]
    )
...
exports.updateSensor = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      'UPDATE sensors SET section_id = ?, sensor_code = ?, sensor_name = ?, sensor_type = ?, limit_max = ?, limit_min = ?, unit = ?, pos_x = ?, pos_y = ? WHERE id = ?',
      [section_id, sensor_code, sensor_name, sensor_type, limit_max, limit_min, unit, pos_x, pos_y, id]
    )
